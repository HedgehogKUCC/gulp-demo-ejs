...
function babel() {
  return gulp.src(envOptions.script.src)
    .pipe($.plumber())
    .pipe($.sourcemaps.init())
    .pipe($.babel({
      presets: ['@babel/env']
    ...
    .pipe($.concat('all.js'))
    .pipe($.sourcemaps.write('.'))
    .pipe(gulp.dest(envOptions.script.path))
    .pipe(
      browserSync.reload({
        stream: true,
      }),
...
function watch() {
  ...
  gulp.watch(envOptions.script.src, gulp.series(babel));
}

exports.deploy = deploy;

exports.build = gulp.series(clean, copyFile, layoutHTML, scss, babel);

exports.default = gulp.series(clean, copyFile, layoutHTML, scss, babel, gulp.parallel(browser, watch));
